...
The script reads `contract.yaml` from the work-package and:
1. Checks every `expected_output` exists in `response/`
2. Verifies directories are non-empty
3. Verifies files are not placeholder stubs
4. Checks `report.md` is substantive
5. Marks strategic checks as `requires_human_review`
